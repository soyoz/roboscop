 ...
 */
class Soyoz_Microsoft_MSN_Container_SB extends Soyoz_Microsoft_MSN_Abstract_Factory
{
    /**
     * @var array
     */
    private $_allowedConfigParameters = array(
        'client' => 'MANDATORY', 'hostname' => 'MANDATORY', 'port' => 'MANDATORY', 'ticket' => 'MANDATORY', 'passport' => 'OPTIONAL', 'sessionId' => 'OPTIONAL'
    );
    ...
    public function factory($config)
    {
        if (isset ($config) && is_array($config)) {
            if (Soyoz_Microsoft_MSN_Adapter_Helper_Check::parameter($config, $this->_allowedConfigParameters)) {
                ...
                $server = 'ss';
                $stream = array(
                    'id' => $sessionId, 'server' => $server, 'passport' => $passport
                );
                ...
                if ($resource && is_resource($resource)) {
                    if (isset ($config ['sessionId'])) {
                        $this->_putANS($sessionId, $server, $ticket, $sessionId);
                    } else {
                        $this->_putUSR($sessionId, $server, $ticket);
                    }
                    ...
                    while (!feof($resource)) {
                        $response = $this->_get($sessionId, $server);
